...
function GlbModel({ url }: { url: string }) {
  ...
  useEffect(() => {
    if (scene) {
      ...
      scene.position.set(-center.x, -center.y + box.max.y * 0.5, -center.z);
    }
  }, [scene]);
    ...
    >
      {glbUrl ? (
        ...
        >
          ...
          <Suspense fallback={null}>{glbUrl && <GlbModel url={glbUrl} />}</Suspense>
          <OrbitControls
            enableDamping
            dampingFactor={0.08}
            minDistance={5}
      ...
      <div className="absolute bottom-4 right-24 text-[10px] text-silver-mist/50 font-medium tracking-wider uppercase">
        {glbUrl ? "OpenCASCADE" : ""}
      </div>
    </motion.div>
  );
