...
> **ManfacterCAD note**: When generating code for `runCadCode`, use `const result = ...` at the end of your code (or `return ...` — both work). Do NOT call `toSTL()`/`toGLB()`/`toSTEP()` — the system handles export automatically. Example: `const base = box(100, 50, 20); const result = difference(base, cylinder(20, 5));`
